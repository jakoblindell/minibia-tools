...
Three opt-in tools on the Spawn Map tab are hidden behind `#stairs-beta` in the URL (e.g. `minibia-tools.html#stairs-beta`) while still being verified against the real map - nothing changes for a normal visit without it:
